						...
						<ul className='friend-list'>
							{notifications && notifications.data
								? notifications.data.length > 0 &&
								  notifications.data.map((item) => <NotiContent item={item} />)
								: 'No Notifications'}
						</ul>
					</div>
				</div>
...
const NotiContent = ({ item }) => {
	return (
		<li key={item.id._id}>
			<Link to='#'>
				<div className='media'>
					<img src={item.id.senderAvatar} alt='user' crossorigin='anonymous' />
					<div className='media-body'>
						<div>
							<h5 className='mt-0'>{item.id.notification}</h5>
							<h6>
								<ReactTimeAgo date={item.id.createdAt} locale='en-US' />
							</h6>
						</div>
					</div>
				</div>
